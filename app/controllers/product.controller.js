...
exports.create = async (req, res, next) => {
    ...
    try {
        ...
        const document = await productService.create(req.body, req.file.filename);
...
exports.findAll = async (req, res, next) => {
    ...
    try {
        ...
        return next(
            new ApiError(500, "An error occurned while retrieving products")
        );
    }
...
exports.update = async (req, res, next) => {
    ...
    try {
        const productService = new ProductService(MongoDB.client);
        const document = await productService.update(req.params.id, req.body, req.file.filename);
        if (!document) {
            return next(new ApiError(404, "Product not found"))
        }
...
exports.updateNoImage = async (req, res, next) => {
    ...
    try {
        const productService = new ProductService(MongoDB.client);
        const document = await productService.updateNoImage(req.params.id, req.body);
        if (!document) {
            return next(new ApiError(404, "Product not found"))
        }
        return res.send({ message: "Product was updated successfully" });
...
// Delete all products off a user from the database
